class VendingMachoneFunc {
    ...
    bindEvents() {
        ...
        this.btnPut.addEventListener('click', () => {
            ...
            if (inputCost) {
                if (inputCost <= myMoneyVal && inputCost > 0) {
                    ...
                    //Iintl 객체를 통해서 사용자 국가에서 사용하는 포맷으로 숫자를 변경
         ...
         * 만약 콜라의 data-cunt값이 0이라면 button요소에 disabled 속성이 추가되고, 콜라 템플릿에 strong 태그가 추가되어야 합니다.
         */
        ...
        colaBtns.forEach((colaBtn) => {
            colaBtn.addEventListener('click', (event) => {
                ...
                if (balanceVal >= targetElPrice) {
                    ...
                    //장바구니에 현재 들어있는 콜라 목록 순회하면서 선택한 콜라가 동일하지 체크
                    const stagedListItem = this.stagedList.querySelectorAll('li')
                    let isStaged = false
                    //for Each문은 모든 요소를 다 돌아서 비효율적이므로 for of이용
                    for (const item of stagedListItem) {
                        //선택한 콜라가 이미 장바구니에 존재하는 경우
                        if (item.dataset.item === colaBtn.dataset.item) {
                            const itmeTxt = item.querySelector('strong')
                            //장바구니 콜라의 카운트를 1증가
                            itmeTxt.firstChild.textContent =
                                parseInt(itmeTxt.firstChild.textContent) + 1
                            isStaged = true
                            break
                        } else {
                    ...
                    if (parseInt(colaBtn.dataset.count) === 0) {
                        //콜라에 disabled속성 추가 + strong 태그 추가
        ...
        /**
         * 4.획득 버튼 기능
         * 획득 버튼을 누르면 선택한 음료수 목록이 음료 목으로 이동한다.
        ...
        this.btnGet.addEventListener("click", () => {
            ...
            // 스프레드 문법으로 한줄로 줄이기 - 기존 forEach에서
            ...
            for (const itemStaged of itemStagedList) {
                ...
                for (const itemGet of itemGetList) {
                    ...
                    if (itemStaged.dataset.item === itemGet.dataset.item) {
                        const itmeTxt = itemGet.querySelector('strong')
                        //획득 목록의 콜라의 카운트를 고른 갯수만큼 증가
                        itmeTxt.firstChild.textContent =
                            parseInt(itmeTxt.firstChild.textContent) +
...
export default VendingMachoneFunc